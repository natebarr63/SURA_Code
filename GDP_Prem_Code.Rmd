...
lmSCR_GDP = lm(as.numeric(GDP) ~ IPO_Gross, data = combined_data_SCR) 
summary(lmSCR_GDP) 

#try with no intercept now (X - 1)
lmSCR_GDP_NOINT = lm(as.numeric(GDP) ~ IPO_Gross - 1, data = combined_data_SCR) 
...
#try with no intercept now (X - 1)
lmSCR_GDPI_NOINT = lm(as.numeric(Gross.private.domestic.investment) ~ IPO_Gross - 1, data = combined_data_SCR) 
